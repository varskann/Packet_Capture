...
cur1.execute('''CREATE TABLE if not exists http(Request_Method text, Request text, Request_Payload text, Response text, Count integer, Content_type text, Content_length integer, Pcap_name text, Item integer)''')
...
for pcap in pcap_files:
    ...
    for item in data:
        number += 1
        cur1.execute('''INSERT INTO http(Request_Method, Request, Request_Payload, Response, Count, Content_type, Content_length, Pcap_name, Item) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)''',(item[0],item[1], item[2], item[3], item[4], item[5], item[6], pcap, number))
    con1.commit()
    con1.close()
...
class CherryProxy_tcpreplay(cherryproxy.CherryProxy):
    ...
    def filter_response(self):
        ...
        cur = con.cursor()
        cur.execute('''SELECT Count, Response, Pcap_name, Content_length FROM http WHERE Request = ?''', (request_uri, ))
        a = cur.fetchone()
        con.close()
        if a != None:
            #print a
            ...
            self.set_response(404, "Response", data = "madmax", content_type='text/plain')
